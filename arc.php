...
$sql = "SELECT * FROM comparchive WHERE 1";
if ($search) {
    // Calculate age dynamically from birthdate and filter by search term
    $sql .= " AND (
...
$sql .= " ORDER BY barangay LIMIT $start_from, $records_per_page";
$result = $conn->query($sql);
...
<body>
    ...
    <a href="archive.php" class="dropdown-toggle"><i class='bx bx-folder'></i> Archive</a>
            <ul class="dropdown-menu">
                ...
                <li class="active"><a href="arc.php" onclick="redirectTo('archive.php')"><i class='bx bx-archive'></i>Complaints archive</a></li>
    ...
    <div class="content">
        <nav>
            ...
            <form action="resarchive.php" method="GET">
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>" . $row["complaint"] . "</td>"; // Display Complaint details
        echo "<td>" . $row["type"] . "</td>";      // Display Type of Complaint
...
?>
    ...
    const complaintsArchiveLink = document.querySelector(".archive-dropdown .dropdown-menu a"); // Target the archive link

    // Check if the current page is "resarc.php" and make the dropdown stay open
    if (window.location.href.includes("arc.php")) {
    ...
    // Prevent dropdown from closing when clicking on any of the items in the dropdown
    complaintsArchiveLink.addEventListener("click", function (e) {
