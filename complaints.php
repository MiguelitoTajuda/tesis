...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    ...
    $filter_query .= " AND (title LIKE ? OR complaint LIKE ? OR barangay LIKE ?)";
    $filter_params[] = $search_term;
    $filter_params[] = $search_term;
    $filter_params[] = $search_term; // Bind the barangay parameter
    $filter_types .= "sss"; // Adding another 's' for the barangay parameter
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_status'])) {
    ...
    if ($stmt->execute()) {
        // If status is "Accomplished," archive the complaint
        if ($status === 'accomplished') {
            // Start a transaction to ensure consistency
            $conn->begin_transaction();

            try {
                // Move the complaint to the archive table
                $sql_insert = "INSERT INTO comparchive (id, title, complaint, type, date, barangay, file, status, complainant, contactnum)
                               SELECT id, title, complaint, type, date, barangay, file, status, complainant, contactnum
                               FROM complaints WHERE id = ?";
                $stmt_insert = $conn->prepare($sql_insert);
                $stmt_insert->bind_param('i', $complaint_id);
                $stmt_insert->execute();

                // Delete the complaint from the complaints table
                $sql_delete = "DELETE FROM complaints WHERE id = ?";
                $stmt_delete = $conn->prepare($sql_delete);
                $stmt_delete->bind_param('i', $complaint_id);
                $stmt_delete->execute();
                ...
                $conn->rollback();
                echo "Error during archiving process: " . $e->getMessage();
            }
        }
